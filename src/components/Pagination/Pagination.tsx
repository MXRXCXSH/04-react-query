...
const buildPages = (
  ...
  const left = Math.max(
    current - Math.floor(safeRange / 2),
    safeMargin
  );
...
const Pagination: React.FC<PaginationProps> = ({
  ...
  const pages = buildPages(total, current, pageRangeDisplayed, marginPagesDisplayed);
    ...
    <ul className={containerClassName} role="navigation" aria-label="Pagination">
      ...
      {pages.map((item, idx) =>
        item === '...' ? (
          ...
          <li key={item} className={item === current ? activeClassName : undefined}>
            <button
              type="button"
              onClick={() => goTo(item)}
